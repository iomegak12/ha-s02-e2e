...
public static class StartupSummaryWriter
{
    ...
    private static void WriteCore(WebApplication app)
    {
        ...
        var baseUrl = cfg["Kestrel:Endpoints:Http:Url"] ?? "http://+:10000";

        var configTable = new Table().Border(TableBorder.Rounded).AddColumns("[bold]Setting[/]", "[bold]Value[/]");
        configTable.AddRow("Service", "nexus-doctor");
        ...
        depsTable.AddRow("Branches client", cfg["Branches:BaseUrl"] ?? "(unset)", "[grey]added in Phase 6[/]");
        depsTable.AddRow("Document storage", cfg["Documents:RootPath"] ?? "(unset)", "[grey]added in Phase 7[/]");
        ...
        togglesTable.AddRow("Swagger UI", "[grey]Phase 6[/]");
        togglesTable.AddRow("ReDoc UI", "[grey]Phase 6[/]");
        togglesTable.AddRow("Prometheus", "[grey]Phase 2[/]");
        togglesTable.AddRow("Document upload", "[grey]Phase 7[/]");
